...
class NewsletterServiceTest {
    ...
    @Test
    void testSaveNewsletterDateAfterNow() {
        newsletter.setNewsletter_date(Date.valueOf(LocalDate.now().plusDays(1)));
        AppUser user = new AppUser();
        when(loggedUser.getUsername()).thenReturn("username");
        when(userRepository.findByUsername("username")).thenReturn(Optional.of(user));
        ...
        assertEquals("Date can't be after current date!", exception.getMessage());
        verify(userRepository, times(1)).findByUsername("username");
        verify(newsletterRepository, never()).save(any(Newsletter.class));
    }
